...
# Try to import wavio, fallback gracefully
try:
    import wavio
    HAS_WAVIO = True
except ImportError:
...
class AudioHandler:
    ...
    def stop_recording(self, output_path: Optional[Path] = None):
        ...
        try:
            ...
            if output_path and self.audio_data and HAS_WAVIO:
                audio_array = np.array(self.audio_data)
                wavio.write(str(output_path), audio_array, 44100, sampwidth=2)
            elif output_path and self.audio_data and not HAS_WAVIO:
                logger.warning("wavio not available, audio file not saved")
            return True
