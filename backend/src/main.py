...
async def main():
    ...
    if ssl_context is None:
        ...
        try:
            ...
            logger.error("generate_cert.py not found. Please create SSL certificates manually.")
            logger.info("You can create self-signed certificates with:")
            logger.info("  openssl req -x509 -newkey rsa:4096 -keyout server.key -out server.crt -days 365 -nodes")
            sys.exit(1)
        except Exception as e:
    ...
    try:
        ...
        logger.error("Could not import StreamingServer. Please check streaming/server_new.py exists")
        sys.exit(1)
    ...
    try:
        ...
        if e.errno == 98 or e.errno == 48:  # Address already in use
            logger.error("Port 5000 is already in use. Please stop the other process or use a different port.")
        else:
