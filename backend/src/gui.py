...
class NodeFlowGUI:
    ...
    def setup_gui(self):
        ...
        # Server URL entry
        ttk.Label(self.status_frame, text="Server URL:").grid(row=1, column=0, padx=5, sticky=tk.W)
        ...
        # Video frame
        self.video_frame = ttk.LabelFrame(self.main_frame, text="Video Preview", padding="5")
        self.video_frame.grid(row=1, column=0, columnspan=2, sticky=(tk.W, tk.E, tk.N, tk.S), pady=5)
        ...
        self.controls_frame.grid(row=2, column=0, columnspan=2, sticky=(tk.W, tk.E), pady=5)
        
        self.stream_btn = ttk.Button(self.controls_frame, text="Start Video", 
                                     command=self.toggle_stream, state='disabled')
        self.stream_btn.grid(row=0, column=0, padx=5)
        
        self.audio_btn = ttk.Button(self.controls_frame, text="Start Audio", 
                                    command=self.toggle_audio, state='disabled')
    ...
    def _run_websocket_thread(self):
        ...
        try:
            ...
            self.root.after(0, lambda: self._update_status("Disconnected", "red", "Connect"))
    ...
    async def _connect_websocket(self):
        try:
            ...
            self.root.after(0, lambda: self._update_status("Connected", "green", "Disconnect"))
            ...
            self.root.after(0, lambda: self._update_status("Connection Failed", "red", "Connect"))
        ...
        try:
            while self.connected and self.ws:
                ...
                if data.get('type') == 'error':
                    ...
                    self.root.after(0, lambda msg=data.get('message'): 
